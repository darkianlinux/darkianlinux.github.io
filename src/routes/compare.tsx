...
export const Route = createFileRoute("/compare")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "How DARKIAN compares to Ubuntu, Kali Linux and Windows 11 in performance, security tools, and privacy." },
    ],
  }),
});
  ...
  return (
    <section className="max-w-6xl mx-auto px-6 py-20">
      ...
      <p className="mt-4 text-muted-foreground max-w-3xl">
        How DARKIAN stacks up against the most popular operating systems for security work.
        Spoiler: <span className="text-primary">DARKIAN wins every category</span>, while Windows 11 finishes last in performance and privacy.
      </p>
        ...
        <table className="w-full text-sm mono">
          <thead className="bg-card">
            <tr className="border-b border-border">
              ...
              {oses.map((os) => (
                <th key={os} className={`text-left p-4 font-semibold ${os === "DARKIAN" ? "text-primary" : ""}`}>
                  {os}
                </th>
              ))}
            </tr>
