...
export const Route = createFileRoute("/compare")({
  ...
  head: () => ({
    meta: [
      { title: "Compare — DARKUNTU vs Ubuntu vs Kali vs Windows 11" },
      { name: "description", content: "How DARKUNTU compares to Ubuntu, Kali Linux and Windows 11 in performance, security tools, and privacy." },
    ],
  }),
});
...
const oses = ["DARKUNTU", "Kali Linux", "Ubuntu", "Windows 11"] as const;
...
function ComparePage() {
  return (
    <section className="max-w-6xl mx-auto px-6 py-20">
      <p className="text-xs mono text-primary mb-2">// benchmarks · tested on Lenovo X1, 16GB RAM, NVMe</p>
      <h1 className="text-4xl md:text-5xl font-bold mono">Differences</h1>
      <p className="mt-4 text-muted-foreground max-w-3xl">
        How DARKUNTU stacks up against the most popular operating systems for security work.
        Spoiler: <span className="text-primary">DARKUNTU wins every category</span>, while Windows 11 finishes last in performance and privacy.
      </p>
        ...
        <table className="w-full text-sm mono">
          <thead className="bg-card">
            <tr className="border-b border-border">
              ...
              {oses.map((os) => (
                <th key={os} className={`text-left p-4 font-semibold ${os === "DARKUNTU" ? "text-primary" : ""}`}>
                  {os}
                </th>
              ))}
            </tr>
            ...
            {rows.map((r) => (
              <tr key={r.feature} className="border-b border-border last:border-0 hover:bg-card/60">
                ...
                {oses.map((os) => {
                  const c = r.values[os];
                  return (
                    <td key={os} className={`p-4 ${os === "DARKUNTU" ? "bg-primary/5" : ""}`}>
                      <div className="flex items-center gap-2">
                        <Indicator score={c.score} />
                        <span className={os === "DARKUNTU" ? "text-foreground" : ""}>{c.label}</span>
                      </div>
                    </td>
                  );
                })}
      ...
      <div className="mt-10 grid md:grid-cols-2 gap-5">
        <div className="rounded-lg border border-primary/40 bg-card p-6">
          <h3 className="font-bold mono text-primary">Why DARKUNTU wins</h3>
          <p className="mt-2 text-sm text-muted-foreground">
            A custom-tuned 6.12 kernel, ZRAM swap, and an aggressively trimmed userland make it
            measurably faster than Kali. With 600+ pre-configured tools, it ships ready for engagements.
          </p>
        </div>
        <div className="rounded-lg border border-border bg-card p-6">
          <h3 className="font-bold mono">Why Windows 11 is last</h3>
          <p className="mt-2 text-sm text-muted-foreground">
            Heavy telemetry, forced updates, mandatory background services and no native pentest tooling
            make it the slowest and least privacy-respecting option for security work.
          </p>
        </div>
      </div>
    </section>
